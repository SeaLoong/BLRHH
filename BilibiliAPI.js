...
var BilibiliAPI = {
    ...
    SilverBox: {
        getAward: function(time_start, end_time, captcha) {
            return BilibiliAPI.lottery.SilverBox.getAward(time_start, end_time, captcha);
        },
        getCaptcha: function(ts) {
            return BilibiliAPI.lottery.SilverBox.getCaptcha(ts);
        },
        getCurrentTask: function() {
            return BilibiliAPI.lottery.SilverBox.getCurrentTask();
        }
    },
    SilverCoinExchange: {
    ...
    Storm: {
        check: function(roomid) {
            return BilibiliAPI.lottery.Storm.check(roomid);
        },
        join: function(id, captcha_token, captcha_phrase, csrf_token, color) {
            return BilibiliAPI.lottery.Storm.join(id, captcha_token, captcha_phrase, csrf_token, color);
        }
    },
    ZongDu: {
        check: function(roomid) {
            return BilibiliAPI.lottery.lottery.check(roomid);
        },
        join: function(roomid, id, csrf_token) {
            return BilibiliAPI.lottery.lottery.join(roomid, id, csrf_token);
        }
    },
    // ajax调用B站API
    last_ajax: 0,
